...
def graph_format(features,output="agg") :
    #features=features.drop(['id'], axis=1).groupby('author').mean().reset_index()
    features = features.set_index("id")
    cols=[col for col in features.columns if col != 'cluster']
    ...
    for cluster in pd.unique(res_df.cluster) :
        ...
        res_df_i = pd.DataFrame.from_dict({"mean":cluster_i_df.mean(),"std":cluster_i_df.std()}).transpose()
        res_df_i["cluster"] = [cluster, cluster]
        final_df = final_df.append(res_df_i.loc['mean'])
        final_df = final_df.append(res_df_i.loc['std'])
        
    return(final_df)

def graph_format2(features, test, output="agg") :
    features = features.set_index("id")
...
def style_spyder_charts(df_results, N_features,test_min = 1e-04, title="MSE Score for Style Evaluation of Embeddings"):
    ...
    for i in range( N_graph) :
        ...
        for d, color, test in zip(np.array(df_results_i.loc[:,spoke_labels]), colors, np.array(df_test_i.loc[:,spoke_labels])):
            ...
            #ax.scatter(theta, d, color=color, s = [- math.log(x+) for x in test if x !=0])
